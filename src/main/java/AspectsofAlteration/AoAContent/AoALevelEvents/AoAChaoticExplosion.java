...
public class AoAChaoticExplosion extends ExplosionEvent implements Attacker {
    protected ParticleTypeSwitcher explosionTypeSwitcher;

    public AoAChaoticExplosion() {
        this(0.0F, 0.0F, 150, BloatedSpiderMob.explosionDamage, false, 0, (Mob)null);
    }

    public AoAChaoticExplosion(float x, float y, int range, GameDamage damage, boolean destructive, int toolTier, Mob owner) {
    ...
    public void spawnExplosionParticle(float x, float y, float dirX, float dirY, int lifeTime, float range) {
        ...
        if (GameRandom.globalRandom.getChance(0.7F) && range < 25.0F) {
            ...
            this.getLevel().entityManager.addParticle(x, y, this.explosionTypeSwitcher.next()).sprite(GameResources.puffParticles.sprite(GameRandom.globalRandom.getIntBetween(0, 4), 0, 12)).sizeFades(70, 80).movesFriction(dx * 0.05F, dy * 0.05F, 0.8F).color(new Color(55, 255, 0)).heightMoves(0.0F, 70.0F).lifeTime(lifeTime * 4);
        }
        ...
        if (range <= (float)Math.max(this.range - 125, 25)) {
            ...
            this.getLevel().entityManager.addParticle(x, y, this.explosionTypeSwitcher.next()).sprite(GameResources.puffParticles.sprite(GameRandom.globalRandom.getIntBetween(0, 4), 0, 12)).sizeFades(30, 50).movesFriction(dx * 0.05F, dy * 0.05F, 0.8F).color(new Color(55, 255, 0)).heightMoves(0.0F, 10.0F).lifeTime(lifeTime * 3);
        }

        if (GameRandom.globalRandom.getChance(0.2F)) {
            this.level.entityManager.addParticle(x, y, this.explosionTypeSwitcher.next()).sprite(GameResources.bubbleParticle.sprite(0, 0, 12)).sizeFades(25, 40).movesConstant(dirX * 0.8F, dirY * 0.8F).flameColor(50.0F).height(10.0F).givesLight(50.0F, 0.2F).onProgress(0.4F, (p) -> {
                Point2D.Float norm = GameMath.normalize(dirX, dirY);
                this.level.entityManager.addParticle(p.x + norm.x * 20.0F, p.y + norm.y * 20.0F, GType.IMPORTANT_COSMETIC).movesConstant(dirX, dirY).smokeColor().heightMoves(10.0F, 40.0F).lifeTime(lifeTime);
            }).lifeTime(lifeTime);
        }
