...
public class AoAIgnusExplosion extends ExplosionEvent implements Attacker {
    protected ParticleTypeSwitcher explosionTypeSwitcher;

    public AoAIgnusExplosion() {
        this(0.0F, 0.0F, 150, BloatedSpiderMob.explosionDamage, false, 0, (Mob)null);
    }

    public AoAIgnusExplosion(float x, float y, int range, GameDamage damage, boolean destructive, int toolTier, Mob owner) {
    ...
    public void spawnExplosionParticle(float x, float y, float dirX, float dirY, int lifeTime, float range) {
        ...
        if (GameRandom.globalRandom.getChance(0.7F) && range < 25.0F) {
            ...
            this.getLevel().entityManager.addParticle(x, y, this.explosionTypeSwitcher.next()).sprite(GameResources.puffParticles.sprite(GameRandom.globalRandom.getIntBetween(0, 4), 0, 12)).sizeFades(70, 80).movesFriction(dx * 0.05F, dy * 0.05F, 0.8F).color(new Color(255, 58, 0)).heightMoves(0.0F, 70.0F).lifeTime(lifeTime * 4);
        }
        ...
        if (range <= (float)Math.max(this.range - 125, 25)) {
            ...
            this.getLevel().entityManager.addParticle(x, y, this.explosionTypeSwitcher.next()).sprite(GameResources.puffParticles.sprite(GameRandom.globalRandom.getIntBetween(0, 4), 0, 12)).sizeFades(30, 50).movesFriction(dx * 0.05F, dy * 0.05F, 0.8F).color(new Color(210, 143, 70)).heightMoves(0.0F, 10.0F).lifeTime(lifeTime * 3);
        }
