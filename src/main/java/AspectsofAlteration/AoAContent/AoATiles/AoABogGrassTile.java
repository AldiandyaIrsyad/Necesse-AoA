...
public class AoABogGrassTile extends TerrainSplatterTile {
    ...
    public AoABogGrassTile() {
        super(false, "aoaboggrasstile");
    ...
    public void addSimulateLogic(Level level, int x, int y, long ticks, SimulatePriorityList list, boolean sendChanges) {
        addSimulateGrow(level, x, y, growChance, ticks, "aoatallboggrassobject", list, sendChanges);
    }
    ...
    public void tick(Level level, int x, int y) {
        if (level.isServer()) {
            if (level.getObjectID(x, y) == 0 && GameRandom.globalRandom.getChance(growChance)) {
                GameObject grass = ObjectRegistry.getObject(ObjectRegistry.getObjectID("aoatallboggrassobject"));
