...
public class AoAAntSpiderNestTile extends TerrainSplatterTile {
    ...
    public AoAAntSpiderNestTile() {
        super(false, "aoaantspidernesttile");
        this.mapColor = new Color(190, 168, 127);
        this.canBeMined = true;
        this.drawRandom = new GameRandom();
    }

    public void addSimulateLogic(Level level, int x, int y, long ticks, SimulatePriorityList list, boolean sendChanges) {
        addSimulateGrow(level, x, y, growChance, ticks, "aoaspiderantegg", list, sendChanges);
    }
    ...
    public void tick(Level level, int x, int y) {
        if (level.isServer()) {
            if (level.getObjectID(x, y) == 0 && GameRandom.globalRandom.getChance(growChance)) {
                GameObject grass = ObjectRegistry.getObject(ObjectRegistry.getObjectID("aoaspiderantegg"));
