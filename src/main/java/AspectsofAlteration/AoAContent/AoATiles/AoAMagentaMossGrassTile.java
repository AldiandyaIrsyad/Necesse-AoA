...
public class AoAMagentaMossGrassTile extends TerrainSplatterTile {
    ...
    public AoAMagentaMossGrassTile() {
        super(false, "aoamagentamosstile");
    ...
    public LootTable getLootTable(Level level, int tileX, int tileY) {
        return new LootTable(new LootItemInterface[]{new ChanceLootItem(0.4F, "aoamagentamossseeditem")});
    }

    public void addSimulateLogic(Level level, int x, int y, long ticks, SimulatePriorityList list, boolean sendChanges) {
        GrassTile.addSimulateGrow(level, x, y, growChance, ticks, "aoamagentamossplantgrassobject", list, sendChanges);
    }
    ...
    public void tick(Level level, int x, int y) {
        if (level.isServerLevel()) {
            if (level.getObjectID(x, y) == 0 && GameRandom.globalRandom.getChance(growChance)) {
                GameObject grass = ObjectRegistry.getObject(ObjectRegistry.getObjectID("aoamagentamossplantgrassobject"));
