...
public class AoAAshenGrassTile extends TerrainSplatterTile {
    ...
    public AoAAshenGrassTile() {
        super(false, "aoaashengrasstile");
        this.mapColor = new Color(171, 171, 171);
        this.canBeMined = true;
        this.drawRandom = new GameRandom();
    }

    public LootTable getLootTable(Level level, int tileX, int tileY) {
        return new LootTable(new LootItemInterface[]{new ChanceLootItem(0.04F, "aoaashengrassseed")});
    }

    public void addSimulateLogic(Level level, int x, int y, long ticks, SimulatePriorityList list, boolean sendChanges) {
        addSimulateGrow(level, x, y, growChance, ticks, "aoavolcanograssobject", list, sendChanges);
    }
    ...
    public void tick(Level level, int x, int y) {
        if (level.isServer()) {
            if (level.getObjectID(x, y) == 0 && GameRandom.globalRandom.getChance(growChance)) {
                GameObject grass = ObjectRegistry.getObject(ObjectRegistry.getObjectID("aoavolcanograssobject"));
