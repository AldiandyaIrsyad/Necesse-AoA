...
import AspectsofAlteration.AoAContent.AoALevelEvents.AoAChaoticExplosion;
...
public class AoAChaoticWave extends Projectile {
    private double distBuffer;

    public AoAChaoticWave() {
    }

    public AoAChaoticWave(Level level, float x, float y, float targetX, float targetY, float speed, int distance, GameDamage damage, Mob owner) {
        this.setLevel(level);
        this.x = x;
        this.y = y;
    ...
    public void onMoveTick(Point2D.Float startPos, double movedDist) {
        ...
        if (this.isServer()) {
            while(this.distBuffer > 150.0) {
                ...
                AoAChaoticExplosion event = new AoAChaoticExplosion(this.x, this.y, 100, getDamage(), false, 0, getOwner());
