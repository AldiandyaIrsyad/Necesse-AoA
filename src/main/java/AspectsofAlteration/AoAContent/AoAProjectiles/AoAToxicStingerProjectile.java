...
public class AoAToxicStingerProjectile extends Projectile {
    public AoAToxicStingerProjectile() {
    }

    public AoAToxicStingerProjectile(Level level, Mob owner, float x, float y, float targetX, float targetY, float speed, int distance, GameDamage damage, int knockback) {
        this.setLevel(level);
        this.setOwner(owner);
        this.x = x;
        this.y = y;
    ...
    public Color getParticleColor() {
        return new Color(183, 0, 255);
    }

    public Trail getTrail() {
        return new Trail(this, this.getLevel(), new Color(87, 17, 122), 16.0F, 500, 16.0F);
    }
    ...
    public void addDrawables(List<LevelSortedDrawable> list, OrderableDrawables tileList, OrderableDrawables topList, OrderableDrawables overlayList, Level level, TickManager tickManager, GameCamera camera, PlayerMob perspective) {
        if (!this.removed()) {
            ...
            int anim = GameUtils.getAnim(this.getWorldEntity().getTime(), 1, 400);
