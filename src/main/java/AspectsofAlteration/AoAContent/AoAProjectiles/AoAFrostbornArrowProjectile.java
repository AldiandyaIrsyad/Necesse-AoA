...
public class AoAFrostbornArrowProjectile extends Projectile {

    public AoAFrostbornArrowProjectile() {
    }

    public AoAFrostbornArrowProjectile(Level level, Mob owner, float x, float y, float targetX, float targetY, float speed, int distance, GameDamage damage, int knockback) {
        this.setLevel(level);
        this.setOwner(owner);
        this.x = x;
        this.y = y;
    ...
    public Trail getTrail() {
        return new Trail(this, this.getLevel(), new Color(43, 175, 150), 6.0F, 20, 16.0F);
    }
    ...
    public void addDrawables(List<LevelSortedDrawable> list, OrderableDrawables tileList, OrderableDrawables topList, OrderableDrawables overlayList, Level level, TickManager tickManager, GameCamera camera, PlayerMob perspective) {
        if (!this.removed()) {
            ...
            int anim = GameUtils.getAnim(this.getWorldEntity().getTime(), 0, 400);
