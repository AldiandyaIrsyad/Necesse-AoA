...
public class AoAFanaticWizardMob extends HostileMob {
    ...
    public void init() {
        super.init();
        PlayerChaserWandererAI<AoAFanaticWizardMob> playerChaserAI = new PlayerChaserWandererAI<AoAFanaticWizardMob>((Supplier)null, 640, 300, 40000, false, false) {
            public boolean attackTarget(AoAFanaticWizardMob mob, Mob target) {
                if (mob.canAttack()) {
                    mob.attack(target.getX(), target.getY(), false);
                        mob.getLevel().entityManager.projectiles.add(new AoAFanaticsVolleyProjectile(mob.getLevel(), mob, mob.x, mob.y, target.x + GameRandom.globalRandom.getIntBetween(100,-100), target.y + GameRandom.globalRandom.getIntBetween(100,-100), 120.0F, 640, new GameDamage(65.0F), 50));
                    mob.getLevel().entityManager.projectiles.add(new AoAFanaticsVolleyProjectile(mob.getLevel(), mob, mob.x, mob.y, target.x + GameRandom.globalRandom.getIntBetween(100,-100), target.y + GameRandom.globalRandom.getIntBetween(100,-100), 120.0F, 640, new GameDamage(65.0F), 50));
                    mob.getLevel().entityManager.projectiles.add(new AoAFanaticsVolleyProjectile(mob.getLevel(), mob, mob.x, mob.y, target.x + GameRandom.globalRandom.getIntBetween(100,-100), target.y + GameRandom.globalRandom.getIntBetween(100,-100), 120.0F, 640, new GameDamage(65.0F), 50));
                    return true;
                } else {
