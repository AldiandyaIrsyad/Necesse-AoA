...
import AspectsofAlteration.AoAContent.AoAProjectiles.AoABossAttackWaveProjectile;
import AspectsofAlteration.AoAContent.AoAProjectiles.AoAChaoticWave;
import AspectsofAlteration.AoAContent.AoAProjectiles.AoADarkflameArrowProjectile;
import AspectsofAlteration.AoAContent.AoAProjectiles.AoAFanaticsVolleyProjectile;
...
public class AoAChaoticMagus extends FlyingBossMob {
    public static LootTable lootTable = new LootTable(new LootItemInterface[]{new ChanceLootItem(0.2F, "milleniumvinyl")});
    public static RotationLootItem uniqueDrops = RotationLootItem.privateLootRotation(new LootItemInterface[]{});
    ...
    public AoAChaoticMagus() {
        ...
        this.magicSoundAbility = (EmptyMobAbility)this.registerAbility(new EmptyMobAbility() {
            protected void run() {
                if (AoAChaoticMagus.this.isClient()) {
                    Screen.playSound(GameResources.magicbolt1, SoundEffect.effect(AoAChaoticMagus.this));
                }
        ...
        this.jingleSoundAbility = (EmptyMobAbility)this.registerAbility(new EmptyMobAbility() {
            protected void run() {
                if (AoAChaoticMagus.this.isClient()) {
                    ...
                    Screen.playSound(GameResources.fadedeath3, SoundEffect.effect(AoAChaoticMagus.this).pitch(pitch));
                }
        ...
        this.attackingAnimationAbility = (BooleanMobAbility)this.registerAbility(new BooleanMobAbility() {
            protected void run(boolean value) {
                AoAChaoticMagus.this.attackingAnimation = value;
            }
        });
    }
    ...
    public void init() {
        super.init();
        this.ai = new BehaviourTreeAI(this, new AoAChaoticMagus.CryoQueenAI(), new FlyingAIMover());
        if (this.isClient()) {
    ...
    public void spawnDeathParticles(float knockbackX, float knockbackY) {
        for(int i = 0; i < 6; ++i) {
            this.getLevel().entityManager.addParticle(new FleshParticle(this.getLevel(), MobRegistry.Textures.fromFile("aoachaoticmagus"), i, 17, 32, this.x, this.y, 10.0F, knockbackX, knockbackY), Particle.GType.IMPORTANT_COSMETIC);
        }
    ...
    protected void addDrawables(List<MobDrawable> list, OrderableDrawables tileList, OrderableDrawables topList, Level level, int x, int y, TickManager tickManager, GameCamera camera, PlayerMob perspective) {
        ...
        DrawOptions body = MobRegistry.Textures.fromFile("aoachaoticmagus").initDraw().sprite(frame, this.attackingAnimation ? 0 : 1, 128).size(128, 128).light(light).mirror(this.dir != 0, false).pos(drawX, drawY);
    ...
    public static class CryoPillar extends GroundPillar {
        ...
        public CryoPillar(int x, int y, double spawnDistance, long spawnTime) {
            ...
            this.texture = MobRegistry.Textures.fromFile("aoachaoticmagus") == null ? null : (GameTextureSection)GameRandom.globalRandom.getOneOf(new GameTextureSection[]{(new GameTextureSection(MobRegistry.Textures.fromFile("aoachaoticmagus"))).sprite(0, 6, 64), (new GameTextureSection(MobRegistry.Textures.fromFile("aoachaoticmagus"))).sprite(1, 6, 64), (new GameTextureSection(MobRegistry.Textures.fromFile("aoachaoticmagus"))).sprite(2, 6, 64)});
    ...
    public static class CryoVolleyRotation3<T extends AoAChaoticMagus> extends RunningAINode<T> {
        private int ticker;
            ...
            GameUtils.streamServerClients(mob.getLevel()).map((c) -> {
                ...
                if (firedAngles.stream().noneMatch((f) -> {
                    ...
                    for(int i = 0; i < missiles; ++i) {
                        ...
                        mob.getLevel().entityManager.projectiles.add(new AoABossAttackWaveProjectile((float)mob.x, (float)mob.y, angleToTarget -20F, (float)2000, (int)2000,AoAChaoticMagus.collisionDamage, mob));
                        mob.getLevel().entityManager.projectiles.add(new AoABossAttackWaveProjectile((float)mob.x, (float)mob.y, angleToTarget -10F, (float)2000, (int)2000,AoAChaoticMagus.collisionDamage, mob));
                        mob.getLevel().entityManager.projectiles.add(new AoABossAttackWaveProjectile((float)mob.x, (float)mob.y, angleToTarget , (float)2000, (int)2000,AoAChaoticMagus.collisionDamage, mob));
                        mob.getLevel().entityManager.projectiles.add(new AoABossAttackWaveProjectile((float)mob.x, (float)mob.y, angleToTarget + 10F, (float)2000, (int)2000,AoAChaoticMagus.collisionDamage, mob));
                        mob.getLevel().entityManager.projectiles.add(new AoABossAttackWaveProjectile((float)mob.x, (float)mob.y, angleToTarget + 20F, (float)2000, (int)2000,AoAChaoticMagus.collisionDamage, mob));
                    }
    ...
    public static class CryoVolleyRotation<T extends AoAChaoticMagus> extends RunningAINode<T> {
        private int ticker;
            ...
            GameUtils.streamServerClients(mob.getLevel()).map((c) -> {
                ...
                if (firedAngles.stream().noneMatch((f) -> {
                    ...
                    for(int i = 0; i < missiles; ++i) {
                        ...
                        Point2D.Float targetPos = new Point2D.Float(target.x + (float)GameRandom.globalRandom.getIntBetween(-50, 50), target.y + (float)GameRandom.globalRandom.getIntBetween(-50, 50));
                        float var10003 = mob.x + offset.x;
                        mob.getLevel().entityManager.projectiles.add(new AoAChaoticWave(mob.getLevel(), mob.x, mob.y, (float)targetPos.x, (float)targetPos.y, 200, 2000, AoAChaoticMagus.collisionDamage, mob));
                    }
    ...
    public static class CryoVolleyRotation2<T extends AoAChaoticMagus> extends RunningAINode<T> {
        private int ticker;
            ...
            GameUtils.streamServerClients(mob.getLevel()).map((c) -> {
                ...
                if (firedAngles.stream().noneMatch((f) -> {
                    ...
                    for(int i = 0; i < missiles; ++i) {
                        ...
                        Point2D.Float targetPos = new Point2D.Float(target.x + (float)GameRandom.globalRandom.getIntBetween(-50, 50), target.y + (float)GameRandom.globalRandom.getIntBetween(-50, 50));
                        float var10003 = mob.x + offset.x;
                        mob.getLevel().entityManager.addLevelEvent(new AoAChaoticBombAttackEvent(mob,(int)target.x + GameRandom.globalRandom.getIntBetween(-300,300), (int)target.y + GameRandom.globalRandom.getIntBetween(-300,300), GameRandom.globalRandom, AoAChaoticMagus.collisionDamage));
                    }
    ...
    public static class CryoShardRotation<T extends AoAChaoticMagus> extends RunningAINode<T> {
        private int ticker;
        ...
        public AINodeResult tickRunning(T mob, Blackboard<T> blackboard) {
            ...
            if (currentTarget != null) {
                ...
                while (this.timerBuffer > msPerShard) {
                    ...
                    mob.getLevel().entityManager.projectiles.add(new AoAFanaticsVolleyProjectile(mob.getLevel(), mob, mob.x, mob.y, currentTarget.x + GameRandom.globalRandom.getIntBetween(100, -100), currentTarget.y + GameRandom.globalRandom.getIntBetween(100, -100), 120.0F, 640, new GameDamage(65.0F), 50));
                    mob.getLevel().entityManager.projectiles.add(new AoAFanaticsVolleyProjectile(mob.getLevel(), mob, mob.x, mob.y, currentTarget.x + GameRandom.globalRandom.getIntBetween(100, -100), currentTarget.y + GameRandom.globalRandom.getIntBetween(100, -100), 120.0F, 640, new GameDamage(65.0F), 50));
                    mob.getLevel().entityManager.projectiles.add(new AoAFanaticsVolleyProjectile(mob.getLevel(), mob, mob.x, mob.y, currentTarget.x + GameRandom.globalRandom.getIntBetween(100, -100), currentTarget.y + GameRandom.globalRandom.getIntBetween(100, -100), 120.0F, 640, new GameDamage(65.0F), 50));
                }
            }
    ...
    public static class CryoShardRotation2<T extends AoAChaoticMagus> extends RunningAINode<T> {
        private int ticker;
        private int timerBuffer;
        private boolean reversed;
        ...
        public AINodeResult tickRunning(T mob, Blackboard<T> blackboard) {
            ...
            if (currentTarget != null) {
                ...
                while(this.timerBuffer > msPerShard) {
                    int speed = 100 + (int)(Math.abs(healthPerc - 1.0F) * 160.0F);
                    this.timerBuffer -= msPerShard;
                    mob.getLevel().entityManager.projectiles.add(new AoADarkflameArrowProjectile(mob.getLevel(), mob, mob.x, mob.y, currentTarget.x, currentTarget.y, 120.0F, 640, new GameDamage(65.0F), 50));
                }
            }
    ...
    public static class CryoQueenAI<T extends AoAChaoticMagus> extends SequenceAINode<T> {
        public CryoQueenAI() {
            ...
            rotations.addChild(new AoAChaoticMagus.CryoShardRotation2());
            rotations.addChild(new AoAChaoticMagus.IdleTime(2000));
            rotations.addChild(new AoAChaoticMagus.CryoVolleyRotation3());
            rotations.addChild(new AoAChaoticMagus.IdleTime(700));
            rotations.addChild(new AoAChaoticMagus.MoveToRandomPosition(true, 300));
            rotations.addChild(new AoAChaoticMagus.IdleTime(100));
            rotations.addChild(new AoAChaoticMagus.CryoShardRotation());
            rotations.addChild(new AoAChaoticMagus.IdleTime(700));
            rotations.addChild(new AoAChaoticMagus.CryoVolleyRotation3());
            rotations.addChild(new AoAChaoticMagus.IdleTime(700));
            rotations.addChild(new AoAChaoticMagus.MoveToRandomPosition(true, 100));
            rotations.addChild(new AoAChaoticMagus.CryoVolleyRotation());
            rotations.addChild(new AoAChaoticMagus.IdleTime(100));
            rotations.addChild(new AoAChaoticMagus.CryoShardRotation2());
            rotations.addChild(new AoAChaoticMagus.IdleTime(2000));
            rotations.addChild(new AoAChaoticMagus.MoveToRandomPosition(true, 300));
            ...
            for(int i = 0; i < 5; ++i) {
                rotations.addChild(new AoAChaoticMagus.MoveToRandomPosition(false, 100));
                rotations.addChild(new AoAChaoticMagus.IdleTime(100));
                rotations.addChild(new AoAChaoticMagus.CryoVolleyRotation2());
            }
