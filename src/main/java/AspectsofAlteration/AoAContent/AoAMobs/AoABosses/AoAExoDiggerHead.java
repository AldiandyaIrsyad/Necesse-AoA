...
public class AoAExoDiggerHead extends BossWormMobHead<AoAExoDiggerBody, AoAExoDiggerHead> {
    ...
    public static RotationLootItem uniqueDrops = RotationLootItem.privateLootRotation(new LootItemInterface[]{new LootItem("razorbladeboomerang"), new LootItem("guardianshell"), new LootItem("dredgingstaff")});
    ...
    public AoAExoDiggerHead() {
        ...
        this.flickSound = (CoordinateMobAbility)this.registerAbility(new CoordinateMobAbility() {
            protected void run(int x, int y) {
                if (AoAExoDiggerHead.this.isClient()) {
    ...
    protected float getDistToBodyPart(AoAExoDiggerBody bodyPart, int index, float lastDistance) {
        return lengthPerBodyPart;
    }

    protected AoAExoDiggerBody createNewBodyPart(int index) {
        Object bodyPart;
        if (index == 69) {
            bodyPart = new AoAExoDiggerTail();
        } else {
            bodyPart = new AoAExoDiggerBody();
        }

        ((AoAExoDiggerBody)bodyPart).sharesHitCooldownWithNext = index % 3 < 2;
        if (index != 0 && index != 68) {
            ((AoAExoDiggerBody)bodyPart).sprite = new Point(index % 4, 0);
            ((AoAExoDiggerBody)bodyPart).shadowSprite = 0;
        } else {
            ((AoAExoDiggerBody)bodyPart).sprite = new Point(4, 0);
            ((AoAExoDiggerBody)bodyPart).shadowSprite = 1;
        }

        return (AoAExoDiggerBody)bodyPart;
    }
    ...
    public void init() {
        super.init();
        this.ai = new BehaviourTreeAI(this, new AoAExoDiggerHead.SwampGuardianAI(), new FlyingAIMover());
        if (this.isClient()) {
            Screen.playSound(GameResources.roar, SoundEffect.globalEffect().pitch(1.2F));
        }
    ...
    public void clientTick() {
        super.clientTick();
        this.getLevel().lightManager.refreshParticleLightFloat(this.x, this.y, 145.0F, -120.0F);
        Screen.setMusic(MusicRegistry.getMusic("battleready"), Screen.MusicPriority.EVENT, 0.8F);
    ...
    public void spawnDeathParticles(float knockbackX, float knockbackY) {
        this.getLevel().entityManager.addParticle(new FleshParticle(this.getLevel(), MobRegistry.Textures.fromFile("aoaexodigger"), GameRandom.globalRandom.nextInt(6), 6, 32, this.x, this.y, 20.0F, knockbackX, knockbackY), Particle.GType.IMPORTANT_COSMETIC);
    }
    ...
    protected void addDrawables(List<MobDrawable> list, OrderableDrawables tileList, OrderableDrawables topList, Level level, int x, int y, TickManager tickManager, GameCamera camera, PlayerMob perspective) {
        ...
        if (this.isVisible()) {
            ...
            WormMobHead.addAngledDrawable(list, new GameSprite(MobRegistry.Textures.fromFile("aoaexodigger"), 0, 1, 96), MobRegistry.Textures.swampGuardian_mask, light, (int)this.height, headAngle, drawX, drawY, 64);
    ...
    public void drawOnMap(TickManager tickManager, int x, int y) {
        ...
        int drawY = y - 24;
        float headAngle = GameMath.fixAngle(GameMath.getAngle(new Point2D.Float(this.dx, this.dy)));
        MobRegistry.Textures.fromFile("aoaexodigger").initDraw().sprite(2, 2, 96).rotate(headAngle + 90.0F, 24, 24).size(48, 48).draw(drawX, drawY);
    }
    ...
    static {
        ...
        MAX_HEALTH = new MaxHealthGetter(25000, 35000, 45000, 50000, 65000);
    }

    public static class SwampGuardianAI<T extends AoAExoDiggerHead> extends SelectorAINode<T> {
        public SwampGuardianAI() {
            ...
            chaserSequence.addChild(new SpawnProjectilesOnHealthLossAINode<T>(AoAExoDiggerHead.totalRazorProjectiles) {
                public void shootProjectile(T mob) {
                    ...
                    if (t != null) {
                        t.bodyPart.getLevel().entityManager.projectiles.add(new SwampRazorProjectile(t.bodyPart.getLevel(), mob, t.bodyPart.x, t.bodyPart.y, t.target.x, t.target.y, 70.0F, 1750, AoAExoDiggerHead.razorDamage, 50));
                        mob.flickSound.runAndSend(t.bodyPart.getX(), t.bodyPart.getY());
                    }

                }
            });
            chaserSequence.addChild(new AoAExoDiggerHead.SpawnBouldersAI(targetFinder));
            chaserSequence.addChild(new AoAExoDiggerHead.DiveChargeRotationAI(chaserAI));
            this.addChild(new WandererAINode(0));
        }
    }

    public static class DiveChargeRotationAI<T extends AoAExoDiggerHead> extends AINode<T> {
        private int ticker;
    ...
    public static class SpawnBouldersAI<T extends AoAExoDiggerHead> extends AINode<T> {
        public int ticker;
        ...
        public AINodeResult tick(T mob, Blackboard<T> blackboard) {
            ...
            if (this.ticker <= 0) {
                ...
                if (t != null) {
                    ...
                    t.bodyPart.getLevel().entityManager.projectiles.add(new SwampBoulderProjectile(t.bodyPart.getLevel(), mob, t.bodyPart.x, t.bodyPart.y, targetPos.x, targetPos.y, 40.0F, dist, new GameDamage(0.0F), 50));
