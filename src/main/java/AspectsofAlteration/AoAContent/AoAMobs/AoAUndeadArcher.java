...
import AspectsofAlteration.AoAContent.AoAProjectiles.AoAFrostbornArrowProjectile;
...
public class AoAUndeadArcher extends HostileMob {
    ...
    public void init() {
        super.init();
        PlayerChaserWandererAI<AoAUndeadArcher> playerChaserAI = new PlayerChaserWandererAI<AoAUndeadArcher>((Supplier)null, 640, 900, 40000, false, false) {
            public boolean attackTarget(AoAUndeadArcher mob, Mob target) {
                if (mob.canAttack()) {
                    mob.attack(target.getX(), target.getY(), false);
                    mob.getLevel().entityManager.projectiles.add(new AoAFrostbornArrowProjectile(mob.getLevel(), mob, mob.x, mob.y, target.x, target.y, 120.0F, 640, new GameDamage(65.0F), 50));
                    return true;
                } else {
                    return false;
                }
