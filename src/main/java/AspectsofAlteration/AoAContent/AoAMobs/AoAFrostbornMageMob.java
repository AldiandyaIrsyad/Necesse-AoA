...
public class AoAFrostbornMageMob extends HostileMob {
    ...
    public void init() {
        super.init();
        PlayerChaserWandererAI<AoAFrostbornMageMob> playerChaserAI = new PlayerChaserWandererAI<AoAFrostbornMageMob>((Supplier)null, 640, 900, 40000, false, false) {
            public boolean attackTarget(AoAFrostbornMageMob mob, Mob target) {
                if (mob.canAttack()) {
                    mob.attack(target.getX(), target.getY(), false);
                    mob.getLevel().entityManager.projectiles.add(new AoAFrostbornSpearProjectile(mob.getLevel(), mob, mob.x, mob.y, target.x + GameRandom.globalRandom.getIntBetween(100,-100), target.y + GameRandom.globalRandom.getIntBetween(100,-100), 120.0F, 640, new GameDamage(65.0F), 50));
                    return true;
                } else {
                    return false;
                }
