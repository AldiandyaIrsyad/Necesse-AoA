...
public class AoAAntSpiderLarva extends HostileMob {

    // Loaded in examplemod.ExampleMod.initResources()
    public static GameTexture texture;

    public static LootTable lootTable = new LootTable(
            ChanceLootItem.between(0.25f, "aoaantplatescale", 1, 1) // 50% chance to drop between 1-3 example items
    );

    // MUST HAVE an empty constructor
    public AoAAntSpiderLarva() {
        super(200);
        setSpeed(50);
        setFriction(3);
    ...
    protected void addDrawables(List<MobDrawable> list, OrderableDrawables tileList, OrderableDrawables topList, Level level, int x, int y, TickManager tickManager, GameCamera camera, PlayerMob perspective) {
        ...
        int drawX = camera.getDrawX(x) - 12;
        int drawY = camera.getDrawY(y) - 21;
