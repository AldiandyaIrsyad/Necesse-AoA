...
public class AoATallBogGrassObject extends GrassObject {
    public static double spreadChance = GameMath.getAverageSuccessRuns(300.0);

    public AoATallBogGrassObject() {
        super("aoatallboggrassobject", -1);
        this.canPlaceOnShore = true;
        this.mapColor = new Color(98, 110, 79);
    ...
    public GameMessage getNewLocalization() {
        return new LocalMessage("object", "aoatallboggrassobject");
    }
    ...
    public String canPlace(Level level, int x, int y, int rotation) {
        return level.getTileID(x, y) != TileRegistry.getTileID("aoaboggrasstile") ? "notmud" : super.canPlace(level, x, y, rotation);
    }

    public boolean isValid(Level level, int x, int y) {
        return super.isValid(level, x, y) && level.getTileID(x, y) == TileRegistry.getTileID("aoaboggrasstile");
    }
