...
public class AoASpiderAntEgg extends GameObject {
    private GameTexture texture;
    private GameTexture brokenTexture;

    public AoASpiderAntEgg() {
    ...
    public void loadTextures() {
        super.loadTextures();
        this.texture = GameTexture.fromFile("objects/aoaantspideregg");
        this.brokenTexture = GameTexture.fromFile("objects/aoaantspideregg_broken");
    }
    ...
    public void onDestroyed(Level level, int x, int y, ServerClient client, ArrayList<ItemPickupEntity> itemsDropped) {
        ...
        if (level.isClient()) {
            level.entityManager.addParticle(new SpiderEggBrokenParticle(level, (float)(x * 32 + 16), (float)(y * 32 + 16), 5000L, this.brokenTexture), Particle.GType.CRITICAL);

            for(int i = 0; i < 40; ++i) {
                level.entityManager.addParticle((float)(x * 32 + 16), (float)(y * 32 + 16), Particle.GType.IMPORTANT_COSMETIC).sprite(GameResources.bubbleParticle.sprite(0, 0, 12)).lifeTime(1000).sizeFades(30, 50).movesFriction((float)(GameRandom.globalRandom.getIntBetween(-10, 10) * (GameRandom.globalRandom.nextBoolean() ? -3 : 3)), (float)(GameRandom.globalRandom.getIntBetween(5, 15) * (GameRandom.globalRandom.nextBoolean() ? -1 : -3)), 0.5F).color(new Color(181, 187, 159)).height(10.0F);
            }
        }
