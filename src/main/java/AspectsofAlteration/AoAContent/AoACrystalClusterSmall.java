...
public class AoACrystalClusterSmall extends GameObject {

    private GameTexture texture;

    public AoACrystalClusterSmall() {
        super(new Rectangle(4, 4, 26, 26)); // Collision relative to the tile it's placed on
        // Remember that tiles are 32x32 pixels in size
        hoverHitbox = new Rectangle(0, -32, 32, 32); // 2 tiles high mouse hover hitbox
        toolType = ToolType.ALL; // Can be broken by all tools
        isLightTransparent = true; // Lets light pass through
        mapColor = new Color(0, 255, 149); // Also applies as debris color if not set
    ...
    public void loadTextures() {
        super.loadTextures();
        texture = GameTexture.fromFile("objects/aoacrystalclustersmall");
    }
...
//        int rotation = level.getObjectRotation(tileX, tileY);
        TextureDrawOptions options = texture.initDraw().light(light).pos(drawX, drawY - texture.getHeight() + 5);
