...
import AspectsofAlteration.AoAContent.AoALegendarySwordRockObjectEntity;
...
public class AoALegendarySwordRock extends GameObject {
    private GameTexture texture;
    private GameTexture brokenTexture;

    public AoALegendarySwordRock() {
    ...
    public void loadTextures() {
        super.loadTextures();
        this.texture = GameTexture.fromFile("objects/aoalegendaryswordrock");
        this.brokenTexture = GameTexture.fromFile("objects/aoalegendaryswordrock_broken");
    }

    public LootTable getLootTable(Level level, int tileX, int tileY) {
        return new LootTable(new LootItemInterface[]{LootItem.between("aoalightbringer", 1, 1)});
    }
    public void addDrawables(List<LevelSortedDrawable> list, OrderableDrawables tileList, Level level, int tileX, int tileY, TickManager tickManager, GameCamera camera, PlayerMob perspective) {
        ...
        if (objectEntity instanceof AoALegendarySwordRockObjectEntity && ((AoALegendarySwordRockObjectEntity)objectEntity).isBroken) {
    ...
    public ObjectEntity getNewObjectEntity(Level level, int x, int y) {
        return new AoALegendarySwordRockObjectEntity(level, x, y);
    }
    ...
    public void onDestroyed(Level level, int x, int y, ServerClient client, ArrayList<ItemPickupEntity> itemsDropped) {
        ...
        if (client != null) {
            ...
            if (objectEntity instanceof AoALegendarySwordRockObjectEntity) {
                AoALegendarySwordRockObjectEntity spiderEggObjectEntity = (AoALegendarySwordRockObjectEntity)objectEntity;
        ...
        if (level.isClient()) {
            ...
            for(int i = 0; i < 120; ++i) {
                level.entityManager.addParticle((float)(x * 32 + 16), (float)(y * 32 + 16), GType.IMPORTANT_COSMETIC).sprite(GameResources.bubbleParticle.sprite(0, 0, 12)).lifeTime(5000).sizeFades(130, 50).movesFriction((float)(GameRandom.globalRandom.getIntBetween(-100, 100) * (GameRandom.globalRandom.nextBoolean() ? -3 : 3)), (float)(GameRandom.globalRandom.getIntBetween(-150, 150) * (GameRandom.globalRandom.nextBoolean() ? -1 : -3)), 0.5F).color(new Color(255, 201, 56)).height(10.0F);
            }
        }
